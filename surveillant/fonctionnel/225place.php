<!doctype html>
 ...
 <p><?php
     ...
     $counter = 0;
      $content = "";
     $statement = $pdo->prepare('SELECT `Image` FROM `appel` WHERE `Place`=\'225\'');
    $statement->execute();
    while ($row = $statement->fetch()) {
     echo '<img src="data:image/jpeg;base64,'.base64_encode( $row['Image'] ).'"/>';
   }
     ?>
  <a class="btn btn-success btn-lg btn-block" href="225Present.php" role="button"  style="margin-top:40px; margin-bottom:20px">Present</a>
 <a class="btn btn-danger btn-lg btn-block" href="225Retard.php" role="button"  style="margin-top:20px">Retardataire</a>
