<html>
    <head>
      <link rel="icon" href="../NouveauLogoStan.png">
      <title>Formulaire de suppression interne </title>
      <link rel="stylesheet" href="../../libs/bootstrap_sans_internet.css">
      <link rel="stylesheet" href="../../style/styleForm.css">
    </head>
    <body>
        <h1>Supression d'un interne</h1>
        <h2>Entrez les données demandées :</h2>
        <h3>Format des jours: lundi, mardi, jeudi, vendredi </h3>
<div class="formulaire">


        <p><form name="Etude" method="post" action="form61.php">

            Place : <input type="text" class="form-control" name="Place" placeholder="Place"/><br/>
            Jour : <input type="text" class="form-control" name="Jour" placeholder="Jour"/><br/>
            <input type="submit" class="btn btn-lg" style="color:black;" name="valider" value="OK"/>
        </form></p>
</div>
    </body>
</html>
